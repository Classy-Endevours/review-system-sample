...
const ReviewSystem = ({ textUrl }) => {
  ...
  const [showIcon, setShowIcon] = useState(false); // To show or hide the comment icon
  ...
  const handleTextSelection = () => {
    ...
    if (selected) {
      ...
      setShowIcon(true); // Show the comment icon
    }
  };
  ...
  const addOrEditComment = () => {
    if (selectedText && comment) {
      ...
      setShowIcon(false); // Hide the comment icon after adding/editing comment
    }
  };
